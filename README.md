...
- [Y] CD into the project base directory `cd reducer-todo`.
- [Y] Create a new react app running `npx create-react-app todo --use-npm`.
- [Y] CD into the react app directory `cd todo`.
- [Y] Start up the app using `npm start`.
- [Y] Create a new branch: git checkout -b `<firstName-lastName>`.
- [I] Implement the project on your newly created `<firstName-lastName>` branch, committing changes regularly.
